...
class Migration1 extends Migration { 
  ...
  Future upgrade() async {
   	database.createTable(SchemaTable("word", [SchemaColumn("id", ManagedPropertyType.bigInteger, isPrimaryKey: true, autoincrement: true, isIndexed: false, isNullable: false, isUnique: false),SchemaColumn("name", ManagedPropertyType.string, isPrimaryKey: false, autoincrement: false, isIndexed: false, isNullable: false, isUnique: true)]));
		...
		database.createTable(SchemaTable("test", [SchemaColumn("id", ManagedPropertyType.bigInteger, isPrimaryKey: true, autoincrement: true, isIndexed: false, isNullable: false, isUnique: false)]));
		...
		database.addColumn("lesson", SchemaColumn.relationship("test", ManagedPropertyType.bigInteger, relatedTableName: "test", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
		database.addColumn("question", SchemaColumn.relationship("questionType", ManagedPropertyType.bigInteger, relatedTableName: "question_type", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
		database.addColumn("question", SchemaColumn.relationship("word", ManagedPropertyType.bigInteger, relatedTableName: "word", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
		database.addColumn("question", SchemaColumn.relationship("character", ManagedPropertyType.bigInteger, relatedTableName: "character", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
		database.addColumn("test", SchemaColumn.relationship("question", ManagedPropertyType.bigInteger, relatedTableName: "question", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
		database.addColumn("theme", SchemaColumn.relationship("lesson", ManagedPropertyType.bigInteger, relatedTableName: "lesson", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
  }
  ...
  Future seed() async {
    ...
    final words = ["Мама", "Папа", "Сын", "Сестра", "Родители"];
    ...
    final questiontypes = ["Попробуйте прописать иероглиф", "Что значит этот иероглиф?"];
    ...
    for (final item in characters) {    
      await database.store?.execute("INSERT INTO character (name) VALUES (@name)", substitutionValues: {
        "name": item
      });
    }

    for (final item in questiontypes) {    
      await database.store?.execute("INSERT INTO question_type (name) VALUES (@name)", substitutionValues: {
        "name": item
      });
    }

    for (final item in questions) {    
      await database.store?.execute("INSERT INTO question (word_id, character_id, questiontype_id) VALUES (@word_id, @character_id, @questiontype_id)", substitutionValues: {
        "word_id": item[0],
        "character_id": item[0],
        "questiontype_id": item[1]
      });
    }

    for (final item in tests) {    
      await database.store?.execute("INSERT INTO test (question_id) VALUES (@question_id)", substitutionValues: {
        "question_id": item
      });
    }

    for (final item in lessons) {    
      await database.store?.execute("INSERT INTO lesson (name, test_id) VALUES (@name, @test_id)", substitutionValues: {
        "name": item[0],
        "test_id": item[1]
      });
    }
    ...
    for (final item in themes) {    
      await database.store?.execute("INSERT INTO theme (name, lesson_id) VALUES (@name, @lesson_id)", substitutionValues: {
        "name": item[0],
        "lesson_id": item[1]
      });
    }
